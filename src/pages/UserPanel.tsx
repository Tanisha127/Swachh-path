...
import { API_KEY as GOOGLE_MAPS_KEY } from '../constants'; // Fallback if env not set
import { 
  APIProvider, 
  Map, 
  AdvancedMarker, 
...
// MCD Locations in Delhi for reference
const MCD_OFFICES = [
...
export default function UserPanel() {
  ...
  useEffect(() => {
    ...
    // Reset tracking for bins that are no longer full
    const fullBinIds = new Set(fullBins.map(b => b.id));
    lastAlertedBins.current.forEach(id => {
      if (!fullBinIds.has(id)) {
        lastAlertedBins.current.delete(id);
      }
    });
  }, [bins]);
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          // Default to Delhi center if blocked
  ...
  const handleVoiceCommand = () => {
    ...
    recognition.onresult = async (event: any) => {
      ...
      const isDryOpen = command.includes('dry open') || command.includes('open dry') || command.includes('सूखा खोलो');
      const isWetOpen = command.includes('wet open') || command.includes('open wet') || command.includes('गीला खोलो');
      const isNearest = command.includes('nearest') || command.includes('find nearest') || command.includes('पास वाला');

      if (isDryOpen) {
        if (selectedBin) {
          try {
            await sendCommand(selectedBin.id, 'dry_open');
            setVoiceFeedback("Command sent: Opening Dry Bin");
          } catch (err) {
            setVoiceFeedback("Failed to send command.");
          }
        } else {
        ...
        if (selectedBin) {
          try {
            ...
            setVoiceFeedback("Command sent: Opening Wet Bin");
          } catch (err) {
            setVoiceFeedback("Failed to send command.");
          }
        } else {
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex flex-col lg:flex-row gap-8 relative">
      ...
      <div className="w-full lg:w-96 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-br from-cyber-accent/5 to-transparent opacity-50"></div>
          <div className="relative flex items-center justify-between mb-4">
            ...
            <div className={`w-2 h-2 rounded-full ${isListening ? 'bg-cyber-danger animate-ping' : 'bg-cyber-success'}`}></div>
          </div>
          ...
          {selectedBin ? (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <BarChart3 className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.usageCount', 'Usage Count')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field1 || 0}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <Hash className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.dryCount', 'Dry Count')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field2 || 0}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <Hash className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.wetCount', 'Wet Count')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field3 || 0}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <Ruler className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.distance', 'Distance')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field4 || 0} cm</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <MapPin className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.latitude', 'Latitude')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field5 || 0}</p>
                </div>
                <div className="p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-cyber-accent/30 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <MapPin className="w-3 h-3 text-slate-500" />
                    <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">{t('bin.longitude', 'Longitude')}</p>
                  </div>
                  <p className="text-sm font-mono text-white">{selectedBin.field6 || 0}</p>
                </div>
                <div className="p-3 bg-cyber-warning/10 rounded-xl border border-cyber-warning/20 group hover:border-cyber-warning/50 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <Droplets className="w-3 h-3 text-cyber-warning" />
                    <p className="text-[8px] font-display text-cyber-warning uppercase tracking-widest">{t('bin.dryFilledLevel', 'Dry Filled Level')}</p>
                  </div>
                  <p className="text-sm font-mono text-cyber-warning">{selectedBin.field7 || 0}%</p>
                </div>
                <div className="p-3 bg-cyber-accent/10 rounded-xl border border-cyber-accent/20 group hover:border-cyber-accent/50 transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <Droplets className="w-3 h-3 text-cyber-accent" />
                    <p className="text-[8px] font-display text-cyber-accent uppercase tracking-widest">{t('bin.wetFilledLevel', 'Wet Filled Level')}</p>
                  </div>
                  <p className="text-sm font-mono text-cyber-accent">{selectedBin.field8 || 0}%</p>
                </div>
              </div>
                  ...
                  <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar pr-2">
                    {routeInfo.steps.map((step: any, idx: number) => (
                      <div key={idx} className="flex items-start gap-2 text-[10px] text-slate-400">
                        <div className="w-1 h-1 rounded-full bg-cyber-accent mt-1.5 flex-shrink-0"></div>
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => sendCommand(selectedBin.id, 'dry_open')}
                  className="flex-1 py-3 bg-cyber-warning/10 hover:bg-cyber-warning/20 text-cyber-warning border border-cyber-warning/30 rounded-xl text-[10px] font-display font-bold uppercase tracking-widest transition-all"
                >
                  {t('user.openDry', 'Open Dry')}
                </button>
                <button 
                  onClick={() => sendCommand(selectedBin.id, 'wet_open')}
                  className="flex-1 py-3 bg-cyber-accent/10 hover:bg-cyber-accent/20 text-cyber-accent border border-cyber-accent/30 rounded-xl text-[10px] font-display font-bold uppercase tracking-widest transition-all"
                >
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-display font-bold text-white uppercase tracking-widest ml-1">{t('user.nearbyBins', 'Nearby Bins')}</h3>
          {nearbyBins.map((bin) => (
            <button
              key={bin.id}
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${bin.wet > 80 || bin.dry > 80 ? 'bg-cyber-danger/10 text-cyber-danger' : 'bg-cyber-accent/10 text-cyber-accent'}`}>
      ...
      <div className="flex-1 relative rounded-3xl overflow-hidden border border-white/5 shadow-2xl">
        <APIProvider apiKey={(import.meta as any).env?.VITE_GOOGLE_MAPS_API_KEY || GOOGLE_MAPS_KEY}>
          ...
          >
            ...
            {userLocation && (
              <AdvancedMarker position={userLocation}>
                <div className="relative">
                  <div className="absolute -inset-4 bg-cyber-accent/20 rounded-full animate-ping"></div>
                  <div className="w-4 h-4 bg-cyber-accent rounded-full border-2 border-white shadow-lg relative z-10"></div>
                </div>
              </AdvancedMarker>
            )}
            ...
            <Directions 
              origin={userLocation} 
              destination={selectedBin?.location} 
        ...
        <div className="absolute bottom-6 left-6 glass-panel p-4 border-white/10 flex items-center gap-6">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-cyber-accent shadow-[0_0_10px_rgba(0,243,255,0.5)]"></div>
            <span className="text-[10px] font-display text-slate-500 uppercase tracking-widest">Available</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-cyber-danger shadow-[0_0_10px_rgba(255,0,60,0.5)]"></div>
            <span className="text-[10px] font-display text-slate-500 uppercase tracking-widest">Critical</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-cyber-warning shadow-[0_0_10px_rgba(255,184,0,0.5)]"></div>
